function [dy] = NovTyson95eqns(t,y)
...
%k1AA = 0.003;   
k3 = 10;     kINH = 0.01;       kCAK = 1;   ka = 0.5;     kbs = 0.2;
...
% dy(10) - change in X
% dy(11) - change in W
% dy(12) - change in Mik1
% dy(13) - change in IEphos
% dy(14) - change in UbE
% dy(15) - change in TAphos
% dy(16) - change in total Cdc25
% dy(17) - change in DNA content
% dy(18) - change in mass
...
dy(7) = kf1*y(nWee1phosr) - ke1*y(nMPFactive)*y(nWee1active) + ((kf2*y(nWee1phosl))/(Kf2 + y(nWee1phosl))) - ((ke2*y(nWee1active))/(Ke2 + y(nWee1active)))*(PK+Nim1)*y(nmass);
dy(8) = ke1*y(nMPFactive)*y(nWee1active) - kf1*y(nWee1phosr) + ((kf2*Wee1phos2)/(Kf2 + Wee1phos2)) - ((ke2*y(nWee1phosr))/(Ke2 + y(nWee1phosr)))*(PK+Nim1)*y(nmass);
dy(9) = kf1*Wee1phos2 - ke1*y(nMPFactive)*y(nWee1phosl) - ((kf2*y(nWee1phosl))/(Kf2 + y(nWee1phosl))) + ((ke2*y(nWee1active))/(Ke2 + y(nWee1active)))*(PK+Nim1)*y(nmass);
dy(10) = kt - kx*y(nX);
dy(11) = ((kh*y(nX)*(Wtotal-y(nW)))/(Kh + Wtotal - y(nW))) - ((kg*y(nW))/(Kg + y(nW)));
dy(12) = kp*y(nW)*(Mik1total - y(nMik1)) - ko*y(nMik1);
dy(13) = ((ki*y(nMPFactive)*(IEtotal - y(nIEphos)))/(Ki + IEtotal - y(nIEphos))) - ((kj*(Tatotal - y(nTaphos))*y(nIEphos))/(Kj + y(nIEphos)));
dy(14) = ((kc*y(nIEphos)*(UbEtotal-y(nUbE)))/(Kc + UbEtotal - y(nUbE))) - ((kd*y(nUbE))/(Kd + y(nUbE)));
dy(15) = ((kk*y(nMPFactive)*(Tatotal - y(nTaphos)))/(Kk + Tatotal - y(nTaphos))) - ((kl*y(nTaphos))/(Kl + y(nTaphos)));
dy(16) = ksyn - kdeg*y(nCdc25total);
dy(17) = V/(1+(F*y(nTaphos)));
dy(18) = mu*y(nmass);
